package com.example.my_kmp_project.component.webview

/**
 * WebView component ownership marker (tpj-flt `tf_webview` analogue).
 *
 * Runtime composable remains [com.example.my_kmp_project.core.platform.PlatformWebView]
 * until expect/actual packages are moved here without breaking OHOS/Android/iOS.
 * Features should import this file's constants/docs path and the platform API via core
 * until the move completes in Spike I person-2 follow-up.
 */
internal object WebViewComponent {
    const val JS_BRIDGE_NAME: String = "NativeBridge"
}

/**
 * Offline-capable Web entry for CI / emulator without outbound net.
 * Percent-encoded `data:` URL keeps Chinese title/body visible without network.
 * Owned by component/webview so feature/home need not import feature/web.
 */
